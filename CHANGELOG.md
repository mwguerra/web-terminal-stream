...
- **Connection stress harness (`composer stress`).** The first registered benchmark: 100 concurrent token-authenticated WebSocket→SSH-PTY sessions (deterministic 3–10s lifetimes, remotely-expanded echo markers proving real round-trips) swept across latency profiles — Toxiproxy on the client leg (`client-10ms`/`client-500ms`) and `tc netem` on the sshd egress (`ssh-10ms`/`ssh-500ms`) plus a baseline — with an over-cap probe asserting `stream.max_connections` refusals (HTTP 503) and a `PtySessionRegistry` drain check after every run. Results merge into `docs/benchmarks/stress-latest.json`. Opt-in via `WTS_STRESS=1`; never runs in CI or `composer bench`. New dev dependency: `ratchet/pawl`.
- **`grid.spec.js` e2e coverage for `TerminalGrid`** — side-by-side `columns(2)` geometry, per-pane WebSocket output isolation, and the pure-CSS `:focus-within` focus ring following clicks between panes.
...
- The bundled JavaScript is now registered with the namespaced Filament asset id `web-terminal-stream` (was `stream-terminal`) so co-installing `mwguerra/web-terminal` cannot collide (Filament keys scripts globally by id). **After upgrading, run `php artisan filament:assets`** so the browser loads the asset at its new published path — otherwise the old file 404s and the terminal fails to boot.
