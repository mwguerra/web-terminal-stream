...
# Register the package's Filament plugin (Terminal Logs resource; the demo
# pages replace the default Terminal page). Must come after ->id() — plugin
# registration reads the panel id. \x5c = backslash, dodging shell escaping.
perl -pi -e "s/->id\\('admin'\\)/->id('admin')\n            ->plugin(\\x5cMWGuerra\\x5cWebTerminalStream\\x5cWebTerminalStreamPlugin::make()->withoutTerminalPage())/" \
