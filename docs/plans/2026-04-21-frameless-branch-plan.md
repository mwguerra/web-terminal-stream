...
- 2026-04-21 — **Playwright validation session**: testapp-f5 pointed at `dev-feature/frameless`, `composer update` ran successfully (all 11 trait files visible in vendor/), `php artisan terminal:serve` started. Drove the real browser against five existing testapp-f5 pages.
  - **Bug caught and fixed**: my earlier edit to `stream-terminal.blade.php` included a literal `"` inside a JS line comment. The outer `x-data="..."` attribute was HTML-truncated by that stray quote, corrupting Alpine's script and producing 10 console errors on every Stream page load. Pest couldn't have caught this — it doesn't render Blade. Fixed in commit `96121ad`.
  - **After the fix** — on every existing regression page:
    - `/admin/stream-terminal`: 0 console errors on load, Alpine state correctly includes `_teardownHandler` (function) and `pendingScriptKey` (string).
    - `/admin/ghostty-terminal` (dual-mode): auto-connected local shell, header + window controls + toggle pill + floating action buttons all render, 0 console errors. Proves `TerminalContainer::instanceId` key-isolation doesn't break the common case.
    - `/admin/terminal` (plugin default): connects cleanly, `CommandValidator` correctly rejects non-whitelisted `echo` ("This command is not permitted."), accepts whitelisted `pwd` and returns `/var/www/test_projects/testapp-f5`. Full Classic round-trip works.
    - `/admin/shell-operator-terminal`: page configured via the now-`@deprecated` `allowPipes/Redirection/Chaining/Expansion` methods — page loads without errors, `whoami` executes and returns `laravel`. Deprecation is PHPDoc-only; runtime behavior unchanged as promised.
  - **Teardown lifecycle validated in-browser**: firing `document.dispatchEvent(new CustomEvent('livewire:navigating'))` runs `destroy()`, calls `ws.close()`, calls `terminal.dispose()`, nulls both references, and clears `_teardownHandler`. This is the exact behavior the Stream buffer bug needed.
...
- 2026-05-13 — **SSH reconnect security fix.** Reproduced the case where disconnecting an SSH-configured terminal and reconnecting silently fell through to a local shell on the app host. Root cause: `disconnect()` was clearing the session-stored connection config, and Livewire does not persist the protected `$connectionConfig` cache across requests, so the next `connect()` resolved to `[]` and the type discriminator defaulted to `local`. Fix is layered: (1) remove the `clearConnectionConfig()` call from `disconnect()` — the session entry is naturally evicted at HTTP-session end, which is the right lifecycle for credentials; (2) add a `#[Locked] public string $designedConnectionType` recorded at mount and checked in `connect()` so any future regression that drops the session config fails closed with a clear error instead of silently downgrading. Two new Pest tests cover both the preserve-on-disconnect behaviour and the type-mismatch refusal. Validated in Playwright across three disconnect/reconnect cycles on testapp-f5 (`ssh-reconnect-{bug-without-fix,fix-verified,three-cycles-verified}.png`).
- 2026-05-13 — **`squareCorners()` appearance knob.** New fluent method on `ConfiguresTerminalAppearance` for users composing several terminals in a CSS grid: when enabled, the outer terminal card drops its `rounded-xl`, `shadow-2xl`, and `ring-1` classes so neighbouring tiles sit flush. Off by default. Plumbed through the Schema component and all three Livewire components (WebTerminal, StreamTerminal, TerminalContainer) so it works in Classic, Stream, and Dual modes. Both Blade views use `@class([...])` to toggle the rounding/ring classes conditionally; CSS rebuilt. Validated in Playwright with a 2×2 tile layout at 1440px and 900px viewports plus a tmux-style nested layout (`tiled-terminals-*.png`).
